...
exports.OnTheFlyAssigner = void 0;
const constants_1 = require("../shared/constants");
const log_1 = require("./log");
const util_1 = require("../shared/util");
class OnTheFlyAssigner {
    constructor(db) {
        this.db = db;
    }
    createAssignment(judgeId) {
        let plausibleIds = this.getPlausibleSubmissionIds(judgeId);
        if (plausibleIds.length < 1) {
            log_1.log("OnTheFlyAssigner: No Plausible Ids!");
            return -1;
        }
        ...
        let submissionsById = util_1.createIdMap(submissions);
        log_1.log(" == OnTheFlyAssigner createAssignment ==");
        log_1.log(`Plausible ids: ${plausibleIds.join(", ")}`);
        log_1.log(`Last location: ${lastLocation}`);
        ...
        let topSubScores = submissionScores.sort((a, b) => Math.sign(b.score - a.score));
        ...
        for (let i = 0; i < 3 && i < topSubScores.length; i++) {
            ...
            log_1.log(`topSubscores[${i}] = ${sub.name} ` +
                `(loc: ${sub.location}) SCORE: ${topSubScores[i].score}`);
        }
        // Create the Assignment
        const priority = this.getNextAssignmentPriority(judgeId);
        const newAssignmentId = this.db.run("INSERT INTO Assignment(judgeId, priority, type, status) " +
            "VALUES(?, ?, ?, ?);", [judgeId, priority, constants_1.ASSIGNMENT_TYPE_RATING, constants_1.ASSIGNMENT_STATUS_ACTIVE]);
        this.db.run("INSERT INTO RatingAssignment(assignmentId, submissionId) " +
            "VALUES(?, ?);", [newAssignmentId, bestSubmissionId]);
        log_1.log(`New assignment created with id ${newAssignmentId}`);
        log_1.log(` --------------`);
     ...
     */
    getLastLocation(judgeId) {
        const lastSubmission = this.db.get("SELECT Submission.location AS location " +
            ...
            "ORDER BY priority DESC;", judgeId);
        if (lastSubmission) {
            return lastSubmission.location;
        }
        else {
            const judge = this.db.get("SELECT anchor FROM Judge WHERE id=?;", judgeId);
    ...
    getSeenSubmissionIds(judgeId) {
        const submissions = this.db.all("SELECT RatingAssignment.submissionId AS id " +
            "FROM RatingAssignment " +
            "LEFT JOIN Assignment ON RatingAssignment.assignmentId = Assignment.id " +
            "WHERE Assignment.judgeId = ?;", judgeId);
        return new Set(submissions.map(sub => sub.id));
    }
    getPlausibleSubmissionIds(judgeId) {
        // We're willing to assign submissions that are not seen and active
        const submissions = this.db.all("SELECT id FROM Submission WHERE active=1;", []);
    ...
    getSubmissions() {
        let dbSubmissions = this.db.all("SELECT id, name, trackId, location, active " +
            "FROM Submission " +
            "ORDER BY id;", []);
        let dbSubPrizes = this.db.all("SELECT submissionId, prizeId, eligibility " +
            "FROM SubmissionPrize " +
            "ORDER BY submissionId;", []);
        let submissions = dbSubmissions.map(sub => (Object.assign(Object.assign({}, sub), { prizeIds: [] })));
        let subIdx = 0;
        for (let subPrz of dbSubPrizes) {
            while (subIdx < submissions.length &&
    ...
    getSubmissionRatings(submissionId) {
        let query = this.db.get("SELECT COUNT(RatingAssignment.id) AS count " +
            "FROM RatingAssignment " +
            "LEFT JOIN Assignment ON Assignment.id=assignmentId " +
            "WHERE submissionId=? AND status=0;", submissionId);
        return query.count;
    }
    getNextAssignmentPriority(judgeId) {
        const highestPriorityAssignment = this.db.get("SELECT priority FROM Assignment " +
            "WHERE judgeId=? " +
            "ORDER BY priority DESC;", judgeId);
    ...
    getPrizeBiases(judgeId) {
        let dbPrizes = this.db.all("SELECT id FROM Prize;", []);
        let map = new Map(dbPrizes.map(prz => [prz.id, 0]));
        let dbBiases = this.db.all("SELECT prizeId, bias " +
            "FROM JudgePrizeBias " +
...
exports.OnTheFlyAssigner = OnTheFlyAssigner;
/**
 ...
 */
function calculateForwardDistancesFrom(allSubmissions, location) {
    ...
    const subsByLoc = allSubmissions.sort((a, b) => Math.sign(a.location - b.location));
    // Find the startIndex, or the first index with the given location.
    let start = 0;
    while (start < subsByLoc.length &&
...
//# sourceMappingURL=OnTheFlyAssigner.js.map