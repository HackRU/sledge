...
exports.BeginJudgingRequest = void 0;
 ...
 */
class BeginJudgingRequest {
    constructor(db) {
        this.db = db;
        this.selectPhase = db.prepare("SELECT phase FROM Status ORDER BY timestamp DESC;");
        this.setPhase = db.prepare("UPDATE Status SET phase=$phase,timestamp=$timestamp;");
        this.countSubmissions = db.prepare("SELECT COUNT() as count FROM Submission;");
        this.countJudges = db.prepare("SELECT COUNT() as count FROM Judge;");
        this.countCategories = db.prepare("SELECT COUNT() as count FROM Category;");
        this.getSubmissionsByLocations = db.prepare("SELECT id, name, location FROM Submission ORDER BY LOCATION;");
    }
    ...
    handleSync(data) {
        ...
        DatabaseHelpers_1.runMany(this.db.prepare("UPDATE Judge SET anchor=? WHERE id=?;"), judgeAnchors.map((loc, j) => [loc, j + 1]));
        // Setup judge prize biases
        (new PrizeBiasMatrix_1.PrizeBiasMatrix(this.db)).setupBiasMatrix();
        // Change phase to 2
        this.setPhase.run({
            phase: 2,
            timestamp: Date.now()
...
exports.BeginJudgingRequest = BeginJudgingRequest;
//# sourceMappingURL=BeginJudgingRequest.js.map